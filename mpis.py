        ...
        try:
            ...
            if option in mkopts("back"):
                ...
                if len(menu[option].childern):
                    ...
                    elif menu.childern[option].name == "Appearance":
                        wizard_config()
                        GlobalMenu.load_appearance()
                    elif menu.childern[option].name == "Set language":
                        set_language()
                    elif menu.childern[option].name == "Toggle --noconfirm":
                        toggle_config("noconfirm")
                    elif menu.childern[option].name == "Toggle multi-install":
                        toggle_config("multi_install")
                    elif menu.childern[option].name == "About MPIS":
                        show_banner()
                        pause()
                    ...
                    elif menu.childern[option].name == "See README File":
                        with open(resource.path_file("README.rst"),
                                  "r") as _file:
                            pause(_file.read())
                    elif menu.childern[option].name == "See CHANGELOG File":
                        with open(resource.path_file("CHANGELOG.rst"),
                                  "r") as _file:
                            pause(_file.read())
                    elif menu.childern[option].name == "See the content of mirrorlist file":
                        with open("/etc/pacman.d/mirrorlist",
                                  "r") as _file:
                            pause(_file.read())
                    else:
                        arg = True if menu.parent.split()[0] == "Install" else False
